...
int	sys_set_pri(envid_t envid, int pri);
int sys_pkt_send(void *addr, int size);
int sys_pkt_recv(void *addr, size_t *size);
int sys_sleep(int channel);
int sys_get_mac_from_eeprom(uint32_t* low, uint32_t* high);
